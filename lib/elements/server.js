...
// Pool öffnen, Query absetzen, Pool schließen und Ergebnis zurück liefern
...
async function deleteElement(tableName, request, response) {
    ...
    try {
        await query(`DELETE FROM ${tableName} WHERE id = $1 AND ownerid = $2;`, [ request.body.id, userId ]);
 ...
 */
async function getElementIds(tableName, request, response) {
    var userId = request.headers['x-user-id'];
    try {
        const result = await query(`SELECT id FROM ${tableName} WHERE ownerid = $1;`, [ userId ]);
...
async function getElementsById(tableName, request, response) {
    var userId = request.headers['x-user-id'];
    try {
        const result = await query(`SELECT id, data FROM ${tableName} WHERE id = ANY($1) AND ownerid = $2;`, [ request.body, userId ]); // https://github.com/brianc/node-postgres/wiki/FAQ#11-how-do-i-build-a-where-foo-in--query-to-find-rows-matching-an-array-of-values
...
async function register(request, response) {
    try {
        ...
        if (resultExisting.rows.length > 0) {
            ...
            var userId = resultCreation.rows[0].id;
            // Owner des Datensatzes auf sich selbst setzen, damit man die data-Infos per API auslesen kann.
            await query(`UPDATE users SET ownerid = $1 WHERE id = $1;`, [ userId ]);
...
async function saveElement(tableName, request, response) {
    var userId = request.headers['x-user-id'];
    if (request.body.id) {
        await query(`UPDATE ${tableName} SET data = $1 WHERE id = $2 AND ownerid = $3 RETURNING id, data;`, [ request.body.data, request.body.id, userId ]);
        response.send(request.body.id);
    } else {
        const result = await query(`INSERT INTO ${tableName} (data, ownerid) VALUES($1, $2) RETURNING id;`, [ request.body.data, userId ]);
...
function registerEndpoint(tableName) {
    ...
    router.post(`/${tableName}/getElementIds`, async (request, response) => getElementIds(tableName, request, response));
...
module.exports = async function(app) {
    ...
    for (var tre of tablesRepresentingEndpoints) {
        ...
        // Tabellen erweitern
        await query(`ALTER TABLE ${tre} ADD COLUMN IF NOT EXISTS ownerid uuid;`);
